...
def get_dataset_info_coco(image_dir, anno_dir, start_id=0, split=None, train_only=True, *args, **kwargs):
    ...
    with ThreadPoolExecutor() as executer:
        results = list(tqdm.tqdm(executer.map(
            coco_image_info, image_info_list), total=len(image_info_list), ncols=100))
...
def make_csv(
    ...
    if val_images_dir or val_anno_dir:
        ...
        train_dataset_df = get_dataset_info(
            image_dir=train_images_dir, anno_dir=train_anno_dir, names_file=names_file, img_to_anno=img_to_anno)
        total_images = len(train_dataset_df.groupby('image_name'))
        val_dataset_df = get_dataset_info(image_dir=val_images_dir, anno_dir=val_anno_dir,
                                          names_file=names_file, img_to_anno=img_to_anno, start_id=total_images, train_only=False)
        ...
        dataset_df = get_dataset_info(image_dir=train_images_dir, anno_dir=train_anno_dir,
                                      names_file=names_file, img_to_anno=img_to_anno, split=split)
    else:
    ...
    meta_into = give_dataset_dict(
        dataset_df, train_images_dir, train_anno_dir, val_images_dir, val_anno_dir)
